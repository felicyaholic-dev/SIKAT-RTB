...
flowchart TD
  ...
  D9[(D9 audit_logs)]
  D10[(D10 password_reset_tokens
  khusus Pengelola, token
  di-hash & sekali pakai)]
  ...
  PGL -->|ID BCA + password,
  isi email di Profil,
  minta reset via email| P2
  P2 --> D6
  P2 -.->|Cek & update kredensial| D2
  P2 --> D10
  P2 -->|Kirim tautan reset| EMAIL
  EMAIL -->|Status kirim| P2
  P2 -->|Sesi login| MHS
  P2 -->|Sesi login| SAT
  P2 -->|Sesi login| PGL
...
- **Proses 3.0** (Pengajuan & Validasi Izin) kini menegakkan beberapa lapis pertahanan tambahan: label `permit_code`/`entry_code` (laporan/riwayat) dibuat lewat generator acak yang aman secara kriptografis (`crypto.getRandomValues`, bukan `Math.random()`); kode/QR yang benar-benar divalidasi satpam berputar tiap 15 detik, bukan nilai tetap (turunan HMAC dari secret per izin `qr_token` + jendela waktu, lihat README §10); dan pencarian kode/QR di gerbang (dipakai P3, ditegakkan lewat D6) kena rate limit terpisah (SCAN, 20 percobaan gagal/15 menit per akun satpam) agar ruang kombinasi kodenya tidak praktis untuk di-brute-force.
