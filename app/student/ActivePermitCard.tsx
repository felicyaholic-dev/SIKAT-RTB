...
export function ActivePermitCard({ permit, qr, code, latestDecision }: { permit: Permit; qr: string; code: string; latestDecision: Decision }) {
  ...
  // Anti-screenshot: the code/QR shown here isn't fixed — it rotates every
  // 15s (see currentPermitCode in lib/db.ts), so this polls a fresh one
  // from the server rather than trusting a value that's already stale by
  // the time the student is looking at it.
  const [snapshot, setSnapshot] = useState({ qr, code });
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        const response = await fetch("/api/student/current-permit-code", { cache: "no-store" });
        if (!response.ok) return;
        const data = await response.json() as { code: string | null; qr: string | null };
        if (active && data.code && data.qr) setSnapshot({ code: data.code, qr: data.qr });
      } catch {
        // Next tick retries — the display just keeps showing the last-known code meanwhile.
      }
    };
  ...
  return (
    <article className="relative min-h-[260px] max-w-xl overflow-hidden border border-line bg-surface p-5 sm:p-6">
      ...
      <div className="border border-line">
        <div className="flex items-center justify-between border-b border-line px-5 py-3"><small className="font-mono text-[10px] tracking-[0.14em] text-muted">{isEntry ? "QR MASUK RTB" : "QR KELUAR RTB"}</small><b className="font-mono text-sm tracking-wide">#{snapshot.code}</b></div>
        <div className="flex flex-col items-center gap-6 p-6">
          <PermitQr svg={snapshot.qr} className="h-52 w-52 sm:h-60 sm:w-60" />
        ...
        <div className="stub-edge px-5 py-4" style={{ "--stub-bg": "var(--color-surface)" } as React.CSSProperties}><p className="text-center text-[11px] text-muted">Pindai atau masukkan kode: <b className="font-mono text-ink">{snapshot.code}</b></p></div>
      </div>

      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-panel bg-amber-soft px-4 py-3">
        <p className="max-w-sm text-[12px] leading-relaxed text-amber"><QrCode size={14} className="mr-1 inline-block" /> QR masih menunggu keputusan satpam. Kode berganti otomatis tiap 15 detik — biarkan halaman ini terbuka saat menunjukkan ke satpam.</p>
        <form action={action}>
