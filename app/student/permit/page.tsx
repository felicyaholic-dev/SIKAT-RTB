...
export default async function StudentPermitPage() {
  ...
  const qr = activePermit ? await generateQrSvg(activePermit.permit_code) : null;

  return (
    <AppShell role="STUDENT" name={session.name}>
      <div className="mx-auto max-w-6xl px-5 py-9 md:px-10 md:py-10">
        <header className="mb-7">
          <p className="font-mono text-[11px] tracking-[0.1em] text-signal">IZIN AKTIF</p>
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-ink md:text-4xl">{activePermit ? `#${activePermit.permit_code}` : "Belum ada izin"}</h1>
          <p className="mt-1 text-sm text-muted">Tunjukkan QR ini kepada satpam saat keluar maupun masuk gerbang.</p>
        </header>
          ...
          {activePermit && qr ? (
            <div>
              <div className="flex items-center justify-between gap-3 pb-4">
                ...
                <span className="shrink-0 text-right text-[11px] text-muted">Rencana kembali {formatDate(activePermit.planned_return_at)}</span>
              </div>

              <div className="border border-line">
                <div className="flex items-center justify-between border-b border-line px-5 py-3">
                  <small className="font-mono text-[10px] tracking-[0.14em] text-muted">IZIN KELUAR</small>
                  <b className="font-mono text-sm tracking-wide">#{activePermit.permit_code}</b>
                </div>
                <div className="flex flex-col items-center gap-6 p-6">
                  ...
                  <dl className="grid w-full max-w-xs grid-cols-2 gap-x-4 gap-y-3.5 text-[12px]">
                    ...
                    <div className="text-center">
                      <dt className="flex items-center justify-center gap-1.5 text-muted">
                        <Clock3 size={13} strokeWidth={1.6} /> Keluar
                      </dt>
                      <dd className="mt-0.5 font-semibold">{formatDate(activePermit.planned_departure_at)}</dd>
                    </div>
                    <div className="text-center">
                      <dt className="flex items-center justify-center gap-1.5 text-muted">
                        <CalendarDays size={13} strokeWidth={1.6} /> Kembali
                      </dt>
                      <dd className="mt-0.5 font-semibold">{formatDate(activePermit.planned_return_at)}</dd>
                    </div>
                  </dl>
                </div>
                <div className="stub-edge px-5 pt-4 pb-4" style={{ "--stub-bg": "var(--color-surface)" } as React.CSSProperties}>
                  <p className="text-center text-[11px] text-muted">
                    Pindai atau masukkan kode: <b className="font-mono text-ink">{activePermit.permit_code}</b>
                  </p>
                </div>
              </div>
