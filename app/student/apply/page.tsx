...
export default async function StudentApplyPage() {
  ...
  const pendingCode = activePermit?.status === "MENUNGGU_MASUK" ? activePermit.entry_code : activePermit?.permit_code;
  ...
  return (
    <AppShell role="STUDENT" name={session.name}>
      <div className="mx-auto max-w-[1180px] px-5 py-9 md:px-10 md:py-11">
        ...
        {waiting && activePermit && qr ? <ActivePermitCard permit={activePermit} qr={qr} latestDecision={latestDecision ?? null} /> : <div className="relative"><PermitForm mode={mode} student={resident} /><StudentPermitDecisionWatcher permitId={decisionPermitId} initialDecision={latestDecision ?? null} /></div>}
      </div>
    </AppShell>
  );
}
