...
export async function GET(request: Request) {
  ...
  const header = ["Kode QR keluar", "Kode QR masuk", "ID BCA", "Nama", "Kamar", "Tujuan", "Waktu keluar (form)", "Waktu kembali (form)", "Keluar tervalidasi", "Masuk tervalidasi", "Status"];
  const csv = [header.map(cell).join(","), ...rows.map((row) => [row.permit_code, row.entry_code, row.bca_id, row.full_name, row.room_number, row.destination, row.planned_departure_at, row.planned_return_at, row.actual_exit_at, row.actual_entry_at, row.status].map(cell).join(","))].join("\n");
